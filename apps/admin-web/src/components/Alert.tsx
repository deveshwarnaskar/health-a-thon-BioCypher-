...
export const Alert: React.FC<AlertProps> = ({
  ...
  return (
    <div
      role="alert"
      className={`rounded-md border p-4 ${configs.bg} ${className}`}
    >
      <div className="flex">
        <div className="flex-shrink-0">{configs.icon}</div>
        <div className="ml-3 flex-1">
          {title && <h3 className={`text-sm font-semibold ${configs.titleColor}`}>{title}</h3>}
          ...
          <div className="ml-auto pl-3">
            <div className="-mx-1.5 -my-1.5">
              <button
                type="button"
                onClick={onDismiss}
                className="inline-flex rounded-md p-1.5 text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2"
