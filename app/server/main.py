...
def create_app(cfg: Settings | None = None, db_path: str | None = None):
    ...
    # webhook). Gemini, when configured, sees only these stored rows. The
    # worker sends its safe follow-up through the same outbound channel as the
    # doctor composer; an optional background poller runs when AAHAAR_AI_INTAKE=on.
    ...
    def debug_status():
        ...
        return {
            ...
            "ai_intake": {
                ...
                "uses_gemini_for_stored_messages": bool(getattr(settings, "ai_intake_use_gemini", True)),
                "interval": getattr(settings, "ai_intake_interval", 15.0),
    ...
    def analyze_status():
        ...
        return {
            ...
            "last_run": intake_worker.last_run,
            "last_summary": intake_worker.last_summary,
            "note": "Reads stored raw_inbound only; Gemini never runs in the Meta webhook. Dashboard reads automatically analyze and send at most one safe follow-up per message.",
        }
