...
async function boot() {
  ...
  const h = await fetch("/health/live").then((r) => r.json()).catch(() => ({}));
  const ch = $("channel");
  if (ch) {
    ch.textContent = "channel: online";
    if (h.status === "healthy" || h.ok === true) ch.classList.add("ok");
  }

  setupTabs();
  setupAddPatient();

  let q = await j("GET", "/api/v2/admin/patients");
  if (!q.ok) {
    q = await j("GET", "/api/v1/patients");
  }
  const patientItems = q.data.patients || (Array.isArray(q.data) ? q.data : []);
  if (q.ok && patientItems.length) {
    state.patients = patientItems.map((p) => ({
      ...p,
      id: p.patient_id || p.id,
      name: p.full_name || p.name,
      phone: p.phone_masked || p.phone,
    }));
    renderPatients();
    selectPatient(state.patients[0].id);
  } else {
    $("patient-list").textContent = "no patients — run seed_dev_stack.py to seed";
  }
}
